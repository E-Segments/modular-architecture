...
/**
 * Module-aware request generator command.
 *
 * Extends Laravel's make:request command to support --module option.
 */
class RequestMakeCommand extends BaseCommand
{
    ...
    {
        if ($this->hasModule()) {
            return $this->getModuleDefaultNamespace($this->getModuleName(), 'request');
        }
    ...
    protected function getPath($name): string
    {
        if ($this->hasModule()) {
            ...
            $namespace = $this->getModuleDefaultNamespace($moduleName, 'request');
            $className = str_replace($namespace.'\\', '', $name);
            $className = str_replace('\\', '/', $className);

            return $this->getModuleFilePath($moduleName, 'request').'/'.$className.'.php';
        }
    ...
    protected function getOptions(): array
    {
        ...
        $options[] = ['module', 'm', InputOption::VALUE_OPTIONAL, 'The module to create the request in'];
