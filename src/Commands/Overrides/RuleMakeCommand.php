...
/**
 * Module-aware rule generator command.
 *
 * Extends Laravel's make:rule command to support --module option.
 */
class RuleMakeCommand extends BaseCommand
{
    ...
    {
        if ($this->hasModule()) {
            return $this->getModuleDefaultNamespace($this->getModuleName(), 'rule');
        }
    ...
    protected function getPath($name): string
    {
        if ($this->hasModule()) {
            ...
            $namespace = $this->getModuleDefaultNamespace($moduleName, 'rule');
            $className = str_replace($namespace.'\\', '', $name);
            $className = str_replace('\\', '/', $className);

            return $this->getModuleFilePath($moduleName, 'rule').'/'.$className.'.php';
        }
    ...
    protected function getOptions(): array
    {
        ...
        $options[] = ['module', 'm', InputOption::VALUE_OPTIONAL, 'The module to create the rule in'];
