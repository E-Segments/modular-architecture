...
/**
 * Module-aware resource generator command.
 *
 * Extends Laravel's make:resource command to support --module option.
 */
class ResourceMakeCommand extends BaseCommand
{
    ...
    {
        if ($this->hasModule()) {
            return $this->getModuleDefaultNamespace($this->getModuleName(), 'resource');
        }
    ...
    protected function getPath($name): string
    {
        if ($this->hasModule()) {
            ...
            $namespace = $this->getModuleDefaultNamespace($moduleName, 'resource');
            $className = str_replace($namespace.'\\', '', $name);
            $className = str_replace('\\', '/', $className);

            return $this->getModuleFilePath($moduleName, 'resource').'/'.$className.'.php';
        }
    ...
    protected function getOptions(): array
    {
        ...
        $options[] = ['module', 'm', InputOption::VALUE_OPTIONAL, 'The module to create the resource in'];
