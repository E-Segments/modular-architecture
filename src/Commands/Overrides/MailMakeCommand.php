...
/**
 * Module-aware mail generator command.
 *
 * Extends Laravel's make:mail command to support --module option.
 */
class MailMakeCommand extends BaseCommand
{
    ...
    {
        if ($this->hasModule()) {
            return $this->getModuleDefaultNamespace($this->getModuleName(), 'mail');
        }
    ...
    protected function getPath($name): string
    {
        if ($this->hasModule()) {
            ...
            $namespace = $this->getModuleDefaultNamespace($moduleName, 'mail');
            $className = str_replace($namespace.'\\', '', $name);
            $className = str_replace('\\', '/', $className);

            return $this->getModuleFilePath($moduleName, 'mail').'/'.$className.'.php';
        }
    ...
    protected function getOptions(): array
    {
        ...
        $options[] = ['module', 'm', InputOption::VALUE_OPTIONAL, 'The module to create the mail in'];
