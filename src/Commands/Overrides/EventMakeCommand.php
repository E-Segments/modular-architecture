...
/**
 * Module-aware event generator command.
 *
 * Extends Laravel's make:event command to support --module option.
 */
class EventMakeCommand extends BaseCommand
{
    ...
    {
        if ($this->hasModule()) {
            return $this->getModuleDefaultNamespace($this->getModuleName(), 'event');
        }
    ...
    protected function getPath($name): string
    {
        if ($this->hasModule()) {
            ...
            $namespace = $this->getModuleDefaultNamespace($moduleName, 'event');
            $className = str_replace($namespace.'\\', '', $name);
            $className = str_replace('\\', '/', $className);

            return $this->getModuleFilePath($moduleName, 'event').'/'.$className.'.php';
        }
    ...
    protected function getOptions(): array
    {
        ...
        $options[] = ['module', 'm', InputOption::VALUE_OPTIONAL, 'The module to create the event in'];
